...
async function Editly(config = {}) {
    ...
    function runTransitionOnFrame({
        ...
        if (transitionName === 'dummy')
            return progress > 0.5 ? toFrame : fromFrame
    ...
    try {
        try {
            ...
            while (true) {
                ...
                const fromClipNumFrames = Math.round(
                    transitionFromClip.duration * fps,
                )
                const toClipNumFrames =
                    transitionToClip &&
                    Math.round(transitionToClip.duration * fps)
                ...
                const fromClipTime =
                    transitionFromClip.duration * fromClipProgress
                const toClipTime =
                    transitionToClip &&
                    transitionToClip.duration * toClipProgress
                ...
                const transitionFrameAt =
                    fromClipFrameAt -
                    (fromClipNumFrames - transitionNumFramesSafe)
                ...
                if (isInTransition) {
                    ...
                    if (frameSource2Data) {
                        const progress =
                            transitionFrameAt / transitionNumFramesSafe
                        const easedProgress =
                            currentTransition.easingFunction(progress)
                ...
                if (!nullOutput)
                    await new Promise((r) =>
                        outProcess.stdin.write(outFrameData, r),
                    )
        ...
        try {
            if (verbose)
                console.log('Waiting for output ffmpeg process to finish')
            await outProcess
        } catch (err) {
